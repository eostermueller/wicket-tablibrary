...
/**
 * Ajaxified version of the paged tabbed panel. Uses AjaxFallbackLink instead of
 * regular wicket links so it can update itself inplace.
 * 
 * @author Igor Vaynberg (ivaynberg)
 * 
 */
public class AjaxPageTabbedPanel extends PageTabbedPanel {
	 ...
	 * @param tabs
	 */
	public AjaxPageTabbedPanel(String id, List<IPageTab> tabs) {
		super(id, tabs);
	...
	protected WebMarkupContainer newLink(String linkId, final int index) {
		return new AjaxFallbackLink(linkId) {
			...
			public void onClick(AjaxRequestTarget target) {
				...
				if (target != null) {
					target.add(AjaxPageTabbedPanel.this);
				}
				onAjaxUpdate(target);
			}
